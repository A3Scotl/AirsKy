...
import iuh.fit.airsky.service.SeatCreationService;
...
public class FlightServiceImpl implements FlightService {
    ...
    public FlightServiceImpl(FlightRepository flightRepository, FlightMapper flightMapper,
            ...
            SeatCreationService seatCreationService) {
    ...
    public FlightResponse createFlight(FlightRequest request) {
        ...
        log.info("=== FLIGHT CREATION STARTED at {} on thread: {} ===", 
                totalStartTime, Thread.currentThread().getName());
        ...
        log.info("Validation completed in {}ms", (validationEndTime - totalStartTime));
        ...
        long entityCreationEndTime = System.currentTimeMillis();
        log.info("Flight entity creation completed in {}ms", (entityCreationEndTime - validationEndTime));
        ...
        long firstSaveEndTime = System.currentTimeMillis();
        log.info("First flight save completed in {}ms", (firstSaveEndTime - entityCreationEndTime));
        ...
        long travelClassGenerationEndTime = System.currentTimeMillis();
        log.info("Travel class generation completed in {}ms", (travelClassGenerationEndTime - firstSaveEndTime));
        ...
        long secondSaveEndTime = System.currentTimeMillis();
        log.info("Second flight save (with travel classes) completed in {}ms", (secondSaveEndTime - travelClassGenerationEndTime));

        // Return response immediately - seats will be created asynchronously
        FlightResponse response = flightMapper.toResponseDTO(finalSaved);
        
        // Enrich with initial available seats (from aircraft capacity) - don't query DB yet
        FlightResponse enrichedResponse = enrichFlightResponseWithInitialAvailableSeats(response, finalSaved);
        
        log.info("About to start async seat creation for flight {} on thread: {}", 
                finalSaved.getFlightId(), Thread.currentThread().getName());

        // Async create seats in background using @Async method
        seatCreationService.createSeatsAsync(finalSaved);
        
        log.info("Returning flight response immediately for flight {} on thread: {}", 
                finalSaved.getFlightId(), Thread.currentThread().getName());

        long totalEndTime = System.currentTimeMillis();
        log.info("=== FLIGHT CREATION COMPLETED in {}ms - RESPONSE READY ===", (totalEndTime - totalStartTime));

        return enrichedResponse;
    }

    /**
     * Enrich FlightResponse với availableSeats cho từng travel class (query DB)
     */
    ...
    /**
     * Enrich FlightResponse với initial availableSeats từ capacity (không query DB)
     * Seats sẽ được tạo async, availableSeats sẽ được update sau
     */
    private FlightResponse enrichFlightResponseWithInitialAvailableSeats(FlightResponse response, Flight flight) {
        if (response.getFlightTravelClasses() != null) {
            for (var ftc : response.getFlightTravelClasses()) {
                // Set initial available seats = capacity (tất cả ghế ban đầu đều available)
                ftc.setAvailableSeats(ftc.getCapacity());
                log.debug("Set initial available seats for travel class {}: {}", 
